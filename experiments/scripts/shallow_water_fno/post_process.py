...
def plot_err_over_time():
    ...
    PlotConfig.save_fig(fig, str(FIG_DIR / "err-over-time-fno"))
...
def main(file: str):
    ...
    if FNO_FLAG:
        # initialize model
        model = FNO2d(log.hparams['modes'], log.hparams['modes'], log.hparams['width'],
    ...
    for j, (y,) in enumerate(tqdm(test_loader)):
        ...
        if j == 0:
            plot_pred(y[0], y_pred[0], 10, "shallow-water-prediction-fno")
