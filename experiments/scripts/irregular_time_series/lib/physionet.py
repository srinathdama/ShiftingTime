...
def variable_time_collate_fn(batch, args, device = torch.device("cpu"), data_type = "train", 
	...
	# observed_tp  = 4*observed_tp - 1 # [0, 0.5] -> [-1, 1]
	# predicted_tp = 4*(predicted_tp - 0.5) - 1 # [0.5, 1] -> [-1, 1]
...
def variable_time_collate_fn_latent_ode(batch, args, device = torch.device("cpu"), data_type = "train", 
	data_min = None, data_max = None, time_max = None):
	"""
	...
	observed_data = combined_vals[:,:n_observed_tp,:]
	observed_mask = combined_mask[:,:n_observed_tp,:]
	predicted_tp = combined_tt[n_observed_tp:]
	predicted_data = combined_vals[:,n_observed_tp:,:]
	predicted_mask = combined_mask[:,n_observed_tp:,:]
