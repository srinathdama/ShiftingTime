...
def _evaluate_metrics_forecasting(model_name, dataloader, model, times, loss_fn, device, kwargs):
    with torch.no_grad():
        ...
        for batch in dataloader:
            ...
            if model_name == 'krno':
                input_x, mask_x, true_y, lengths = batch                
                pred_y = model(input_x, mask_x)
            else:
        ...
        print(f"Eval iterations {len(time_taken)} took {avg_time_per_iter:.4f} seconds per iteration.")
...
def _train_loop_forecasting(model_name, train_dataloader, val_dataloader,test_dataloader, model, times, optimizer, loss_fn, eval_fn, max_epochs,
    ...
    plateau_terminate = 200
    tqdm_range = tqdm.tqdm(range(max_epochs))
    tqdm_range.write('Starting training for model:\n\n' + str(model) + '\n\n')

    if model_name == 'krno':
        out_times   = times[-1] + torch.linspace(1, model.output_time, model.output_time).to(device)
        in_times    = times / out_times[-1]
        out_times   = out_times/ out_times[-1]

        ## get quad grids
        quad_grid_in = quadrature.trapezoidal_vecs_uneven(in_times.to('cpu'))
        quad_grid_out = quadrature.trapezoidal_vecs_uneven(out_times.to('cpu'))

        quad_grid_in = ([quad_grid_in[0]], [quad_grid_in[1]])
        quad_grid_out = ([quad_grid_out[0]], [quad_grid_out[1]])

        model.quad_grid_in = quadrature.quad_grid_to_device(quad_grid_in, device)
        model.quad_grid_out = quadrature.quad_grid_to_device(quad_grid_out, device)
    ...
    for epoch in tqdm_range:
        ...
        for batch in train_dataloader:
            ...
            with _SuppressAssertions(tqdm_range):
                ...
                if model_name == 'krno':
                    train_x, mask_x, train_y, lengths = batch                
                    pred_y = model(train_x, mask_x)
                else:
        ...
        avg_time_per_iter = np.mean(time_taken)
        print(f"Train Iiterations {len(time_taken)} took {avg_time_per_iter:.4f} seconds per iteration.")
        ...
        if epoch % epoch_per_metric == 0 or epoch == max_epochs - 1:
            ...
            tqdm_range.write(' Best model | Val loss: {:.3} | Test loss : {:.3}'.format(best_val_loss, best_test_loss))
...
def make_model(name, input_channels, output_channels, hidden_channels, hidden_hidden_channels, 
    ...
    ##    
        ...
        def make_model():
            ...
            model = models.KRNO(hparams, output_time)
            return model, model
    else:
