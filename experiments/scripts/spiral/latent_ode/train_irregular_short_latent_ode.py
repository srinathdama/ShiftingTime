...
parser.add_argument('-s', '--sample-tp', type=float, default=50, help="Number of time points to sub-sample."
...
parser.add_argument('--dataset', type=str, default='cyclic_traj', help="Dataset to load. Available: cyclic_traj")
...
def compute_test_metrics(model, seq_data, seq_times, device, past_time, future_time, delta_t, sample_freq, flag='val') -> dict[str, float]:
    ...
    for start_time in start_times:
        ...
        ##
        batch_dict = {}
        batch_dict["tp_to_predict"] = y_ind # (L_out)
        batch_dict["observed_data"] = x.reshape(1, -1, 2)  # X (B=1, L_in, N=2) 
        batch_dict["observed_tp"]   = x_ind # (L_in)
        batch_dict["observed_mask"] = torch.ones_like(batch_dict["observed_data"]).to(device) #(B=1, L_in, N=2)
        batch_dict["data_to_predict"] = y.reshape(1, -1, 2)  # (B, L_out, N=2)
        batch_dict["mask_predicted_data"] = None # (B, L_out, N=2)
        batch_dict["mode"]    = 'extrap'
        batch_dict["labels"]  = None

        kl_coef = 0.0
        n_traj_samples = 1
        train_res, y_pred = model.compute_all_losses(batch_dict, n_traj_samples, kl_coef)
...
def compute_forecast_metrics(model, seq_data, seq_times, device,
    ...
    for i, start_time in enumerate(start_times):
        if i % skip_steps == 0:
            ...
            ##
            if i == 0:
                y_prev = x.reshape(1, -1, 2)  # X (B=1, M=1, L_in, N=2)
            batch_dict["observed_data"] = y_prev
            batch_dict["observed_mask"] = torch.ones_like(batch_dict["observed_data"]).to(device) #(B=1, L_in, N=2)
            train_res, y_pred = model.compute_all_losses(batch_dict, n_traj_samples, kl_coef)
            y_prev = y_pred.reshape(y.shape)
...
def train_one_traj(idx, samp_irregular_traj_i, samp_irregular_ts_i,
    ...
    model = create_LatentODE_model(args, input_dim, z0_prior, obsrv_std, args.device, 
        classif_per_tp = classif_per_tp,
        n_labels = n_labels).to(args.device)
