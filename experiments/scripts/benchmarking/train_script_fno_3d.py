...
def main(lag: int, res: int, fno_modes: int, nepochs: int):
    seed = 42
    ...
    if FNO_FLAG:
        ...
        # initialize model
        # model = FNO3d(hparams['modes'], hparams['modes'], int(lag/2), hparams['width'],
        #             hparams['in_channels'], hparams['out_channels']).to(device)
        model = FNO3d(hparams['modes'], hparams['modes'], 6, hparams['width'],
                    hparams['in_channels'], hparams['out_channels']).to(device)
    else:
        hparams = dict(
            d=3,
    ...
    for epoch in range(1, n_epochs + 1):
        for xx, yy in train_loader:
            ...
            if FNO_FLAG:
                ...
                y_pred = model(cart_grid, xx)
                loss = loss_fn(y_pred, yy, cart_grid)
            loss.backward()
            optimizer.step()
            if lr_decay_flag:
                scheduler.step()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fno_modes", "-m", type=int, default=12)
    ...
    main(args.lag, args.res, args.fno_modes, args.nepochs)
