...
DATA_DIR = os.path.join(root_base_path, 'dataset/elasticity')
...
parser.add_argument("--res", "-r", type=int, default=16) # 16, 24, 32, 41
...
if train_flag:
    path_model     = os.path.join(CKPT_DIR, f'KRNO_{args.res}_{optim}_{date_time}.pt')
else:
...
hparams = dict(
    ...
    affine_in_first_integral_tsfm=False
)
...
if train_flag:
    for ep in range(epochs):
        ...
        for x, y in train_loader:
            ...
            out = model.super_resolution(quad_grid_out, quad_grid_in, x, quad_grid_latent)
            out = out*mask
    ...
    save_best_res_path  = os.path.join(CKPT_DIR, f'test_results_{S}.npz')
